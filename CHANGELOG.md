...
- **Striking-distance query pass.** `seo-executor.md` gains Template E: from a GSC Performance export in `{plan-dir}/data/`, take the top pages by impressions, pull queries at position 4-15 with >= 50 impressions whose wording is missing from title/H1/headings/first 100 words, apply a cannibalization guard and an intent guard, add at most 3 surviving queries per page in a heading or early paragraph, and log page/query/impressions/position-before so the result is measurable. `seo-auditor.md` CONTENT checklist grows from 12 to 13 items with the striking-distance table (or a documented "no export" gap). `seo-measurer.md` gains a content check that scores the pass by per-query position delta against a fresh export (NOT VERIFIED, not FAIL, when no export exists). `seo-planner-agent.md` schedules a `striking-distance` quick-win task when the audit table has rows. New `{plan-dir}/data/` directory (created by bootstrap, documented in the `SKILL.md` layout) holds user-supplied exports. `SKILL.md` content-audit description and EXECUTE phase 2 list the new item.
